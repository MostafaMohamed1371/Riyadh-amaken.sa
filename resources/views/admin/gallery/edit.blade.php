...
<div class="space-y-6">
    ...
    <x-admin.card>
        <x-slot:header><h2 class="text-lg font-semibold text-slate-800">Edit Gallery Item</h2></x-slot:header>
        <form action="{{ route('admin.gallery.update', $gallery) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    @if($gallery->image)
                        <p class="mb-1.5 text-xs text-slate-500">Current: <img src="{{ asset('storage/' . $gallery->image) }}" alt="" class="inline h-10 w-16 rounded-lg object-cover align-middle"></p>
                    @endif
                ...
                <div>
                    ...
                    <input type="url" name="link" id="link" value="{{ old('link', $gallery->link) }}" class="mt-1.5 block w-full rounded-xl border-slate-300 bg-white px-4 py-2.5 shadow-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20">
                </div>
            </div>
